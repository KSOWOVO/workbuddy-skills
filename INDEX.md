...
| `03-data-analysis/file-content-inspection` | 判断文件能否删之前**先真正打开看内容**（禁止只看文件名） | 打开看看、里面是什么、这文件是什么、能不能删、哈希名文件、Download (1).mp4 是什么、帮我看看这批文件 | 7.5K | 覆盖 PDF(乱码 CMap)/Office/HTML/视频/压缩包/未知扩展名；**用户「文件名看着没用」几乎总是误判**（哈希名 xlsx 其实是问卷原始数据） |
...
| `03-data-analysis/safe-duplicate-cleanup` | **安全清理重复文件**：MD5 扫真重复 → 避开程序必需文件 → 一律移入回收站 | 重复文件、去重、删重复、同样的文件存了两份、微信占空间、C盘满了、清理冗余、MD5 相同 | 10.0K | Windows 向、偏保守：**绝不永久删除**；跨文件名重复识别；避开运行中程序的依赖 |
...
| `01-browser-automation/cnki-institutional-download` | **知网机构权限批量下载 + 题录复核**：Playwright 驱动系统 Edge（不下载浏览器）+ 六大坑规避 | 知网下载、CNKI、批量下文献、下载全文、查知网、题录核验、校园网、机构权限、图书馆权限、下不动文献 | 4.3K | 核心：**校园网 IP 已认证就不用登录**；`korder=TI` 篇名精确检索；`downloads_path`+等 `.crdownload` 消失；每篇独立开页+保活页；文件名 sanitize；**不写滑块破解** |
...
| `04-content-generation/docx-paper-audit-revision` | 已有论文 DOCX 的**定稿审计与批改**：数据复算 + 引用链体检 + AI 味量化改写 + 内嵌图替换 | 改论文、论文定稿、帮我核一下论文、引用有没有问题、AI 味、去 AI 腔、参考文献核验、幻影引用、孤儿文献、论文体检、图换掉 | 6.1K | 四项审计；**只改不改写观点**，每条改动留备份链；与 survey-to-journal-paper 分工：那个是「写」，这个是「审」 |
...
| `04-content-generation/video-to-obsidian-cuda` | 视频/音频 → **CUDA 转录 → Obsidian 笔记 + ima 备份**；含全篇校对与手写小标题 | 转录、转写、转录这个视频、视频转文字、用 CUDA 转录、丢视频、课程视频整理、转写稿、把视频存进 Obsidian | 14.4K | faster-whisper large-v3 + CUDA；**强制两步整理**：①全篇通读改同音错词（重聚→从句、编语→宾语…）②小标题必须手写（词频法只出草稿）；截图默认关（`--shots` 开启） |
| `04-content-generation/survey-to-journal-paper` | 问卷统计结果 + 文风底座 + 期刊模板 → **中文核心期刊格式实证论文 DOCX**（三线表/模型图/中英摘要） | 写论文、把数据写成论文、实证论文、期刊格式、三线表、SEM 论文、按这个模板写、文风仿写、投稿初稿、**把数据加进已有文章/文章不能改只加数据**、**只留第X部分/只汇报数据分析/按那篇论文格式排版/汇报稿打印稿** | 6.4K | **三模式**：A 写新论文 / B 融进既有论文（原文只增不改）/ C 抽取章节重排汇报稿（**正文一字不改 + 表号绝不重编**）；含 `scripts/docx_kit.py`（字体/三线表/引号修复）+ `references/docx-cn-typesetting.md`（中文引号转直引号、禁对 py 源码做引号替换、列宽反算齐版心）+ `references/merge-into-existing-paper.md`（**模式B**：md5 锁源文件 / 章号顺延 run 拆分坑 / 附录版"后文"指代漂移 / 双版本交付 / **分号必须为 0** / 池化SD 与 CR-AVE 口径）+ `references/extract-chapters-rebuild.md`（**模式C**：body 顺序抽取 / 文字列左对齐判据只看数据行 / **表格校验必须走 XML——row.cells 合并展开会虚报差异**） |
| `04-content-generation/live2d-texture-restyle` | **Live2D 皮套改造 + 画风对齐**：贴图分区 → 画风量化 → 约束式提示词 → 批量筛查 → 拼回贴图 | 皮套改造、换皮、改模型、OC 改造、Live2D 改图、贴图重绘、画风对齐、AI 画风和原画不一致、风格统一、虚拟形象改造 | 10.1K | 含 `scripts/part_segment.py`、`style_probe.py`、`reassemble.py`、`batch_scan.py`（**四个均已实测跑通**）；**五大核心：① moc3 锁死几何只能换贴图 ② 新部件剪影受网格 UV 限制（外套做不出 oversized） ③ 要保留原脸就直接不碰脸的分区 ④ 画风必须先量化（V/S/线条占比/主色）再写 prompt ⑤ 大件底色必须浅色，深色会砸明度触发"油画感"**；素材：`style_study/mesh_uv_overview.png`（网格 UV 岛总览） |
| `04-content-generation/docx-template-injection` | **作业/课程报告模板 docx 注入**（保留封面/信息表/分工表/页眉）+ 自制图表 + 案例素材图 | 填进模板、按模板做、作业模板、课程作业、保留封面、小组分工表、图文并茂、老师发的模板、填进去、交作业 | 16.5K | **两个致命坑**：① python-docx 搬运 XML 元素 → 图片关系留临时文档，Word 里全部显示不出来（必须直接在目标文档创建段落/图片）；② 模板说明文字要连 `w:highlight yellow` 一起清。含 `scripts/`：inject_docx / fill_members / clean_highlight / verify / make_figs_v2 / download_assets（B站封面+Steam截图）/ compress_assets |
| `04-content-generation/single-file-dashboard-audit` | **多板块单文件 HTML 看板的数据完整性自检**：防「数据写了但进不了卡片/筛选/搜索」+ 去重合并规范 | 没看到你放进去、数据没显示、筛选里没有、搜不到、卡片缺了、数据遗漏巡检、看板自检、字段规范化、去重合并 | 7.0K | 六大自检层（数据层/渲染层/属性层/标签层/格式层/覆盖层）；含三大去重坑（`&`≡`and`、方向后缀不参与裸名匹配、裸名撞方向族改名）；**大文件 HTML 别靠截图验证，用静态字符串分析** |
| `90-tooling/skill-github-backup` | 自创 skill 同步 GitHub | skill 备份、同步、开源、上 GitHub | 5.1K | 含 `scripts/sync_to_github.py` |
| `90-tooling/skill-router` | 路由元决策 + **同类 skill 权重仲裁** + **复合任务编排（6 条管道）** | 该用哪个 skill、要不要建 skill、该自己写脚本吗、同类撞车选谁、写论文/做数据该串哪几个 | 3.2K | 含 `scripts/skill_match.py` + `weights.json`（**调路由改这个，不动脚本**）+ **`references/combos.md`（C1 论文生产 / C2 问卷数据 / C3 知识库 / C4 跨会话 / C5 skill 运维 / C6 浏览器取证，含交接物与坑）** |
...
| `05-system-utils/phone-audio-to-pc` | **手机声音 → Windows 电脑（纯音频，不投屏）**：scrcpy `--no-video` 道 + 方向坑 + bat 编码坑 | 手机声音到电脑、手机音频转发、听不到手机声音、手机声音进耳机、电脑当手机音箱、音频流转、scrcpy | 4.5K | **先判方向**：AudioRelay/SoundWire 等全是反方向（PC→手机），传不了手机系统音频；`--no-window` ≠ 不传视频 |
...
04-content-generation/   简报、写作、日报、插画动图（GIF）、看板自检
