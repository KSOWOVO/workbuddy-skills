...
| `04-content-generation/live2d-texture-restyle` | **Live2D 皮套改造 + 画风对齐**：贴图分区 → 画风量化 → 约束式提示词 → 拼回贴图 | 皮套改造、换皮、改模型、OC 改造、Live2D 改图、贴图重绘、画风对齐、AI 画风和原画不一致、风格统一、虚拟形象改造 | 6.6K | 含 `scripts/part_segment.py`、`style_probe.py`、`reassemble.py`（**均已实测跑通**，2048 贴图 round-trip 通过）；**三大核心：① moc3 锁死几何只能换贴图 ② 画风必须先量化（V/S/线条占比/主色）再写进 prompt ③ 加外套等大件底色必须浅色，深色会砸掉明度触发"油画感"** |
