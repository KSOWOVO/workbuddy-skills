...
| `04-content-generation/docx-template-injection` | **作业/课程报告模板 docx 注入**（保留封面/信息表/分工表/页眉）+ 自制图表 + 案例素材图 | 填进模板、按模板做、作业模板、课程作业、保留封面、小组分工表、图文并茂、老师发的模板、填进去、交作业 | 6.3K | 8.2K | **两个致命坑**：① python-docx 搬运 XML 元素 → 图片关系留临时文档，Word 里全部显示不出来（必须直接在目标文档创建段落/图片）；② 模板说明文字要连 `w:highlight yellow` 一起清。含 `scripts/`：inject_docx / fill_members / clean_highlight / verify / make_figs_v2 / download_assets（B站封面+Steam截图）/ compress_assets |
