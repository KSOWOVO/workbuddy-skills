...
### ⚠️ 改编号时，有 4 个地方必须同步（漏一个就穿帮）

| # | 位置 | 怎么发现漏了 |
|---|---|---|
| 1 | 正文章节里的引用（"如图4所示"） | grep `图\d` |
| 2 | 文档里的图注段落 | 遍历 docx 段落 |
| 3 | **图片内部印的标题** | **必须打开 PNG 看**——最容易漏！ |
| 4 | 图片文件名 | `ls figs/` |

**实测事故**：图注从「图一~图六」改成「图1~图9」后，忘了重新生成 PNG，导致**图片里印着「图四」而文档图注写着「图6」**，用户一眼看出来了。
**对策**：图表生成脚本里把编号写成变量，且**文件名直接带图号**（`fig6_interaction.png`），改一次全局一致。

⚠️ **注入时的另一个经典 bug**：正文若提前提到「见文末图9」，注入脚本会**在那里就把图9插进去**。要么别提前引用，要么把「代码见图9」这类表述放到真正的位置。
