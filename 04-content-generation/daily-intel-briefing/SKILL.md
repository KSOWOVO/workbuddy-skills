...
agent_created: true
summary: 生成「全球宏观·AI·科技硬件·游戏」英文日报（v6：IELTS 6.5 / 高中 3500 词难度），HTML 主交付：英文界面+每段折叠中文全文翻译；选中整句走在线意译、单词查内置词典；英文句 hover 联动中文句高亮；指数卡按 PE 十年分位 30/70 给买入/持有/卖出信号。
description: >
  ...
  ⑤HTML 交互：选中整句(≥3词)→MyMemory 整句意译；单词→DICT 400+ 秒查→未命中在线；英文句 hover→对应中文句浅亮平滑放大；指数卡点击弹走势图；
...
- 主交付：同名 `.html`（`Global_Macro_Tech_Gaming_Intel_YYYY-MM-DD.html`），纯内联 CSS/JS 零外部依赖，必须包含：
  1. **选中翻译**：`#dict-pop` 气泡 + `DICT` 423+ 词对象（normWord 归一化）+ mouseup 监听——**整句(≥3英文词或2词且>20字符)直接 MyMemory 整句意译，不再逐词；单词先查 DICT 未命中走在线**。
  2. **hover 联动**：加载时按句子切分 `.p-en`/`.p-cn` 成 `.en-s`/`.cn-s` span，hover 英文句 → 对应中文句 `.lit` 浅亮+平滑放大（mouseenter/mouseleave；段落含 `.term` 时跳过重建）。
